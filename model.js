...
//						myControl.util.dump(' -> errorThrown = '+errorThrown);
	...
	*/
	...
	//				});
				...
				myControl.util.dump(' -> dispatched Q ('+QID+') is empty');
				//an empty queue.
				}
...
//			myControl.util.dump(' -> namespace: '+extensionObjItem.namespace);
			...
			var r = true; //set to false if extension contains any errors or file can't be loaded.
...
//immediately after the data is loaded into the control, the extension.namespace.callbacks.init is run.  This is where any extension specific dependencies or system settings should be checked/reported.
...
//it means that a developer could use an extension that didn't load properly, but that is their perogative, since we told them its broke.
						...
						if(typeof myControl.extensions[namespace].callbacks.init === 'object')	{
							initPassed = myControl.extensions[namespace].callbacks.init.onSuccess(); //returns t/f
							}
						else	{
...
//do not execute the callback if errors were generated in load process.
						if(!errors && callback)	{
...
//				myControl.util.dump(' -> valid extension object containing '+myControl.model.countProperties(extensionObj)+' extensions');
	...
	//namespace and filename are required for any extension.
					if(!extensionObj[index].namespace || !extensionObj[index].filename)	{
						...
						if(myControl.model.fetchExtension(extensionObj[index]))	{
							myControl.util.dump(" -> successfully loaded ext: "+extensionObj[index].namespace);
							}
						else	{
							myControl.util.dump(" -> WARNING! ext NOT loaded: "+extensionObj[index].namespace)
							}
