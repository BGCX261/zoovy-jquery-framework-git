	...
	calls : {
		addGiftcardToCart : {
			...
			dispatch : function(giftcard,callback)	{
				myControl.model.addDispatchToQ({"_cmd":"addGiftcardToCart","giftcard":giftcard,"_tag" : {"callback":callback,"extension":"convertSessionToOrder"}},'priorityDispatchQ');	
				}			
			}, //addGiftcardToCart
		...
		addCouponToCart : {
			...
			dispatch : function(coupon,callback)	{
				myControl.model.addDispatchToQ({"_cmd":"addCouponToCart","coupon":coupon,"_tag" : {"callback":callback,"extension":"convertSessionToOrder"}},'priorityDispatchQ');	
				}			
			}, //addCouponToCart
...
//update will modify the cart. only run this when actually selecting a shipping method (like during checkout). heavier call.
		getShippingRatesWithUpdate : {
			...
			dispatch : function(callback)	{
				myControl.model.addDispatchToQ({"_cmd":"getShippingRates","update":"1","trace":"1","_tag": {"datapointer":"getShippingRates","callback":callback,"extension":"convertSessionToOrder"}},'priorityDispatchQ');
				}
			}, //getShippingRatesWithUpdate
		...
		getCustomerAddresses : {
			...
			dispatch : function(callback)	{
				myControl.model.addDispatchToQ({"_cmd":"getCustomerAddresses","_tag": {"datapointer":"getCustomerAddresses","callback":callback,"extension":"convertSessionToOrder"}},'priorityDispatchQ');
				}
			}, //getCustomerAddresses	
		...
		getCheckoutDestinations : {
			...
			dispatch : function(callback)	{
				myControl.model.addDispatchToQ({"_cmd":"getCheckoutDestinations","_tag": {"datapointer":"getCheckoutDestinations","callback":callback,"extension":"convertSessionToOrder"}},'priorityDispatchQ');
				}
			}, //getCheckoutDestinations
...
//payment methods for other use should use an extension specific or generic call.
		getPaymentMethodsForCheckout : {
			...
			dispatch : function(callback)	{
				...
				myControl.model.addDispatchToQ({"_cmd":"getPaymentMethods","_tag": {"datapointer":"getPaymentMethods","callback":callback,"extension":"convertSessionToOrder"},"country":country,"ordertotal":total},'priorityDispatchQ');
				}
			}, //getPaymentMethods	

		getCartContents : {
			init : function(callback)	{
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.calls.getCartContents. callback = '+callback)
				this.dispatch(callback);
				},
			dispatch : function(callback)	{
//				myControl.util.dump(' -> adding to PDQ. callback = '+callback)
				myControl.model.addDispatchToQ({"_cmd":"showCart","_zjsid":myControl.sessionId,"_tag": {"datapointer":"showCart","callback":callback,"extension":"convertSessionToOrder"}},'priorityDispatchQ');
				}
			},//getCartContents
...
//				myControl.model.addDispatchToQ({"_cmd":"validateCheckoutProperties","_tag" : {"callback":callback,"extension":"convertSessionToOrder"}},'priorityDispatchQ');
				}
			}, //saveCheckoutFields
			
		showCheckoutForm : {
			init : function()	{
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.calls.showCheckoutForm.init');
				myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutPreflight');
...
//multiple inv_mode by 1 to treat as number
				if((zGlobals.globalSettings.inv_mode * 1) > 0)	{ 
					myControl.extensions.convertSessionToOrder.calls.verifyCartInventory.init("handleInventoryUpdate");
					}
				myControl.extensions.convertSessionToOrder.calls.getPaymentMethodsForCheckout.init();
				myControl.extensions.convertSessionToOrder.calls.getCustomerAddresses.init();
				myControl.extensions.convertSessionToOrder.calls.getCheckoutDestinations.init();
				myControl.extensions.convertSessionToOrder.calls.getShippingRatesWithUpdate.init();
				myControl.extensions.convertSessionToOrder.calls.getCartContents.init("loadPanelContent");
				}
			}, //showCheckoutForm
...
// exp alone is less valuable, so it's stored in data.cart obj and referenced there.
				payObj['payment.cc'] = myControl.extensions.convertSessionToOrder.vars["payment.cc"];
				payObj['payment.cv'] = myControl.extensions.convertSessionToOrder.vars["payment.cv"];
				payObj['payment.yy'] = myControl.extensions.convertSessionToOrder.vars["payment.yy"];
				payObj['payment.mm'] = myControl.extensions.convertSessionToOrder.vars["payment.mm"];
				...
				myControl.model.addDispatchToQ(payObj,'priorityDispatchQ');
				}
			},//createOrder
...
*/
		processCheckout : {
			init : function(callback)	{
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.calls.processCheckout.init');
				...
				$('#chkoutPlaceOrderBtn').attr('disabled','disabled').addClass('disabled loadingButtonBg'); //disable the button to avoid double-click.
//				return; //die here to test
				var checkoutIsValid = myControl.extensions.convertSessionToOrder.validate.isValid();
...
//originally, instead of attr(disabled,false) i removed the disabled attribute. This didn't work in ios 5 safari.					
					$('#chkoutPlaceOrderBtn').attr('disabled',false).removeClass('disabled').removeClass('loadingButtonBg');
...
//all items have available inventory (if needed based on store settings).
		verifyCartInventory : {
			...
			dispatch : function(callback)	{
				myControl.model.addDispatchToQ({"_cmd":"verifyCartInventory","_tag": {"datapointer":"verifyCartInventory","callback":callback,"extension":"convertSessionToOrder"}},'priorityDispatchQ');
				}
			} //verifyCartInventory		
...
//callbacks.init need to return either a true or a false, depending on whether or not the file will execute properly based on store account configuration.
		init : {
			onSuccess : function()	{
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.init.onSuccess');
...
//messaging for the test harness 'success'.
					...
					$('#zContent').removeClass('loadingBG').append("");
					}
				else	{
					
//generates the checkout container div and FORM.
					$('#zContent').append(myControl.renderFunctions.createTemplateInstance('zCheckoutContainerSpec','zCheckoutContainer'));
//myControl.data is passed in because something needs to be, but this is generated prior to any ajax calls occuring (possibly) so the cart can't be passed in. !!! can we just pass in an empty object? seems better. test this.
					myControl.renderFunctions.translateTemplate(myControl.data,'zCheckoutContainer');

					myControl.extensions.convertSessionToOrder.calls.showCheckoutForm.init();
					myControl.model.dispatchThis("priorityDispatchQ");
...
//errors are reported to the screen, but 'other' actions may need to be taken.
				...
				return r;
//				myControl.util.dump('END myControl.extensions.convertSessionToOrder.init.onSuccess');
				},
			onError : function()	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.init.error');
				//This would be reached if a templates was not defined in the view.
				$('#zContent').removeClass('loadingBG');
				}
			}, //init
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.updateCheckoutShipMethods.success');
				myControl.extensions.convertSessionToOrder.panelContent.shipMethods();
				},
			onError : function(d)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.updateCheckoutShipMethods.onError - ERROR!');
				myControl.extensions.convertSessionToOrder.panelContent.shipMethods(); //reload ship panel or just error shows and there is no way to continue.
...
//after a gift card is entered, update the payment panel as well as the cart/invoice panel.
				myControl.extensions.convertSessionToOrder.panelContent.cartContents();
				myControl.extensions.convertSessionToOrder.panelContent.paymentOptions();
...
_gaq.push(['_trackEvent','Checkout','User Event','Cart updated - giftcard added']);
			...
			onError : function(d)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.addGiftcardToCart.onError - ERROR!');
				myControl.extensions.convertSessionToOrder.panelContent.paymentOptions(); //regenerate the panel. need to show something or no payments can be selected.
		...
		addCouponToCart : {
			onSuccess : function(tagObj)	{
				myControl.util.dump('BEGIN control.extensions.convertSessionToOrder.callbacks.addcouponToCart.onSuccess');
				$('#addCouponBtn').removeAttr('disabled').removeClass('disabled').removeClass('loadingButtonBg');
...
_gaq.push(['_trackEvent','Checkout','User Event','Cart updated - coupon added']);
			...
			onError : function(d)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.addcouponToCart.onError');
				$('#addCouponBtn').removeAttr('disabled').removeClass('disabled').removeClass('loadingButtonBg');
		...
		authenticateZoovyUser : {
			onSuccess : function(tagObj)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.authenticateZoovyUser.onSuccess');
				$('#chkout-create_customer').val(0);
...
_gaq.push(['_trackEvent','Authentication','User Event','Logged in through Store']);
			...
			onError : function(d)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.authenticateZoovyUser.onError');
				$("#chkoutPreflightFieldsetErrors").toggle(true).append("<li>It appears that username/password is invalid. Please try again or continue as a guest.<\/li>");
...
//					myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.handleInventoryUpdate.onSuccess');
...
_gaq.push(['_trackEvent','Checkout','App Event','Cart updated - inventory adjusted to reflect availability']);
				...
				onError : function(d)	{
					myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.handleInventoryUpdate.onError - ERROR!');
					myControl.extensions.convertSessionToOrder.panelContent.paymentOptions();
//global errors are emptied when 'complete order' is pushed, so do not empty in the responses or any other errors will be lost.
		...
		updateCheckoutPayOptions : {
			onSuccess : function(tagObj)	{
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.updateCheckoutPayOptions.success');
				myControl.extensions.convertSessionToOrder.panelContent.paymentOptions();
				},
			onError : function(d)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.updateCheckoutPayOptions.onError - ERROR!');
				myControl.extensions.convertSessionToOrder.panelContent.paymentOptions();  //reload panel or just error shows and user can't proceed.
				$('#chkoutPayOptionsFieldsetErrors').append(myControl.util.getResponseErrors(d)).toggle(true);
				}
			},
		...
		updateCheckoutOrderContents : {
			onSuccess : function(){
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.updateCheckoutOrderContents.success (updating cart panel)');
				myControl.extensions.convertSessionToOrder.panelContent.cartContents();
				},
			onError : function(d)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.updateCheckoutOrderContents.onError - ERROR!');
				myControl.extensions.convertSessionToOrder.panelContent.cartContents();  //reload panel so more than just error shows up and user can proceed/try again.
...
*/
		finishedValidatingCheckout : {
			onSuccess : function(tagObj)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.finishedValidatingCheckout.onSuccess');
//for payby, used what is in the form, NOT what is set in the cart/session (it may not have been updated prior to this dispatch being executed).
				myControl.extensions.convertSessionToOrder.calls.createOrder.init("checkoutSuccess");
				myControl.model.dispatchThis('priorityDispatchQ');
...
_gaq.push(['_trackEvent','Checkout','App Event','Server side validation passed']);
			...
			onError : function(d)	{
				$('#chkoutPlaceOrderBtn').removeAttr('disabled').removeClass('disabled loadingButtonBg'); //make place order button appear and be clickable.
				myControl.extensions.convertSessionToOrder.utilities.showServerErrors(d);  //displays error messages.
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.finishedValidatingCheckout.onError.  Errors = ');
...
_gaq.push(['_trackEvent','Checkout','App Event','Server side validation failed']);
		...
		loadPanelContent : {
			onSuccess : function(tagObj)	{
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.loadPanelContent.onSuccess');
...
//				myControl.util.dump('stuff util.countProperties = '+stuffCount+' and typeof = '+typeof stuffCount);
				...
				if(stuffCount > 0)	{
					...
					myControl.extensions.convertSessionToOrder.panelContent.preflight();
...
//ship address will make itself visible if user is authenticated.
						myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutAccountInfo');
						myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutBillAddress');

//if billtoship == 1, then hide the panel (pass in true). if it's off, show the panel (pass in false)
						myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutShipAddress',myControl.data.showCart.cart['chkout.bill_to_ship']*1 > 0?true:false);
						myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutShipMethods'); 
						myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutPayOptions');
						myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutOrderNotes',true);						
//populate panels.						
						myControl.extensions.convertSessionToOrder.panelContent.cartContents();
						myControl.extensions.convertSessionToOrder.panelContent.billAddress();
						myControl.extensions.convertSessionToOrder.panelContent.accountInfo();
						myControl.extensions.convertSessionToOrder.panelContent.shipAddress();
						myControl.extensions.convertSessionToOrder.panelContent.shipMethods();
						myControl.extensions.convertSessionToOrder.panelContent.paymentOptions();
//if order notes is on, show panel and populate content.
						if(zGlobals.checkoutSettings.chkout_order_notes == true)	{
							myControl.extensions.convertSessionToOrder.panelContent.orderNotes();
...
//if focusFieldset is not set, then this is the first time a panel/fieldset change has occured.
							if(!myControl.util.isSet(myControl.extensions.convertSessionToOrder.vars.focusFieldset))	{
								myControl.extensions.convertSessionToOrder.vars.focusFieldset = e.currentTarget.id;
								}
							else if(e.currentTarget.id != myControl.extensions.convertSessionToOrder.vars.focusFieldset)	{
								myControl.util.dump('FIELDSET changed! old = '+myControl.extensions.convertSessionToOrder.vars.focusFieldset+' and new = '+e.currentTarget.id);
								myControl.extensions.convertSessionToOrder.validate[myControl.extensions.convertSessionToOrder.vars.focusFieldset](); //validate the fieldset that was just left.
								myControl.extensions.convertSessionToOrder.vars.focusFieldset = e.currentTarget.id; //set currentfieldset to the new fieldset id.
								}
							else	{
...
_gaq.push(['_trackEvent','Checkout','Milestone','Valid email address obtained']);
					...
					myControl.extensions.convertSessionToOrder.utilities.cartIsEmptyWarning();
					}
				$('#zContent').removeClass('loadingBG');
				},
			onError : function(d)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.loadPanelContent.onError.');
				...
				$('#globalMessaging').append(myControl.util.getResponseErrors(d)).toggle(true);
//				$('#zContent').empty().removeClass('loadingBG').append("<div id='unableToCheckoutError' class='zwarn'>");
				}
...
*/
		checkoutSuccess : {
			onSuccess : function(tagObj)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.checkoutSuccess.onSuccess   datapointer = '+tagObj.datapointer);
//nuke old form content. not needed anymore. gets replaced with invoice-ish content.
				var $zContent = $('#zContent').empty();
				var oldSession = myControl.sessionId;
				var orderID = myControl.data[tagObj.datapointer].orderid;

				myControl.util.jumpToAnchor('zContent');
...
//				$('.paymentRequired').append(myControl.extensions.convertSessionToOrder.utilities.checkoutSuccessPaymentFailure(myControl.data[tagObj.datapointer].payment_success,myControl.data['order|'+orderID].cart['chkout.payby']));
				
				
				
				var cartContentsAsLinks = encodeURI(myControl.extensions.convertSessionToOrder.utilities.cartContentsAsLinks('order|'+orderID))
				...
				myControl.extensions.convertSessionToOrder.calls.getCartContents.init(); //!IMPORTANT! will reset local cart object. 
				myControl.model.dispatchThis('priorityDispatchQ'); //these are auto-dispatched because they're essential.
...
_gaq.push(['_trackEvent','Checkout','User Event','Order created ('+orderID+')']);
			...
			onError : function(d)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.callbacks.checkoutSuccess.onError   datapointer = '+tagObj.datapointer);
				myControl.extensions.convertSessionToOrder.utilities.showServerErrors(d);
...
//				myControl.util.dump('END myControl.extensions.convertSessionToOrder.validate.isValid. sum = '+sum);
				if(sum != 6)	{
					r = false;
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.validation.chkoutPreflightFieldset');
...
//						myControl.util.dump(' -> email is blank');
						...
						myControl.extensions.convertSessionToOrder.vars.validPreflight = 0;
...
//				myControl.util.dump('END myControl.extensions.convertSessionToOrder.validation.chkoutPreflightFieldset');
				return valid;
...
//no validation occurs here by default, but function exists so it can be overridden AND because there's some automated scripting which validates each panel as the user moves on.
			...
			chkoutAccountInfoFieldset : function()	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.validation.chkoutAccountInfoFieldset');
				var valid = 1;
...
//in addition to selecting a pay option, certain extra fields may be present and must be checked for.
			chkoutPayOptionsFieldset : function()	{
				var valid = 1;
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.validate.chkoutPayOptionsFieldset');
				...
				if($payMethod.val())	{
					switch($payMethod.val())	{
						...
						case 'ECHECK':
							for(var key in myControl.extensions.convertSessionToOrder.vars.echeck) {
								$holder = $('#'+myControl.util.makeSafeHTMLId(key)).removeClass('mandatory');
								if(!$holder.val())	{
									$holder.parent().addClass('mandatory');
									valid = 0;
									errMsg += '<li>please enter a '+myControl.extensions.convertSessionToOrder.vars.echeck[key]+'<\/li>'
									}
								}
							break;
...
//				myControl.util.dump('END myControl.extensions.convertSessionToOrder.validate.chkoutPayOptionsFieldset. paymethod = '+$payMethod.val()+' and valid = '+valid);
				return valid;
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.validate.address');
...
*/
			preflight : function()	{
//				myControl.util.dump("BEGIN myControl.extensions.convertSessionToOrder.panelContent.preflightPanel.");
...
//						myControl.util.dump(" -> Already Authenticated");
						...
						o += "<input type='hidden' class='zform_textbox zform_email'  name='data.bill_email' id='data-bill_email' value='"+email+"' /><\/ul>";
						}
					else	{

//						myControl.util.dump(" -> Login Prompts (default panel behavior/else)");
						o = "<div id='preflightGuestInputs' class='preflightInputContainer'><div class='ztitle'>Guest Checkout<\/div><div><label for='data.bill_email'>Email<\/label>";
						o += "<input type='email' class='zform_textbox zform_email'  name='data.bill_email' id='data-bill_email' value='"+email+"' onkeypress='if (event.keyCode==13){$(\"#guestCheckoutBtn\").click();}' />";
						o += "<button class='zform_button' onClick='myControl.extensions.convertSessionToOrder.utilities.handleGuestEmail($(\"#data-bill_email\").val());' id='guestCheckoutBtn'>"
...
//NOTE - IE prefers onClick not onChange on checkboxes.
						o += "<div id='chkout-create_customerContainer'><input class='zform_checkbox' type='checkbox' checked='checked' value='1' onClick=\"myControl.extensions.convertSessionToOrder.utilities.handleCreateAccountCheckbox(this.checked?1:0);\" name='chkout.create_customer_cb' id='chkout-create_customer_cb' \/> <label for='chkout-create_customer_cb'>Create Customer Account<\/label><\/div>"
	...
	//for guests and thirdPartyCheckin, the native login form is hidden. it's still rendered so we can toggle it as needed.
						...
						o += "<div id='preflightAccountInputs' class='"+className+" preflightInputContainer'><div class='ztitle'>Existing Users<\/div>";
						o += "<div><label for='data.bill_email2'>Email<\/label><input type='email' class='zform_textbox zform_email'  name='data.bill_email' id='data-bill_email2' value='"+email+"' /><\/div>";
						o += "<div><label for='chkout-password'>Password<\/label><input type='password' class='zform_textbox zform_password'   name='password' id='chkout-password' value='' onkeypress='if (event.keyCode==13){$(\"#userLoginBtn\").click();}' />";
						o += "<button  class='zform_button' onClick='myControl.extensions.convertSessionToOrder.utilities.handleUserLogin($(\"#data-bill_email2\").val(),$(\"#chkout-password\").val());' id='userLoginBtn'>Log in<\/button><\/div><\/div>";
	...
	//only show the third party logins IF user isn't already logged in to one and they're enabled
						if(authState != 'thirdPartyGuest' && zGlobals.thirdParty.facebook.appId)
								o+= "<div id='preflightThirdParty' class='"+className+" preflightInputContainer'><div class='ztitle'>Third Party Login<\/div><fb:login-button onlogin='myControl.calls.authentication.facebook.init({\"callback\":\"authenticateThirdParty\",\"extension\":\"convertSessionToOrder\"}); myControl.model.dispatchThis(\"priorityDispatchQ\");'>Login with Facebook</fb:login-button><\/div>";
						}
...
//				myControl.util.dump('END myControl.extensions.convertSessionToOrder.panelContent.preflighPanel.');
				}, //preflight
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.panelContent.accountInfo.  ');
				var authState = myControl.sharedCheckoutUtilities.determineAuthentication();
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.panelContent.billAddress.  ');
				...
				var cssClass; //used to hide the form inputs if user is logged in and has predefined addresses. inputs are still generated so user can create a new address.
			 	var authState = myControl.sharedCheckoutUtilities.determineAuthentication();
				if(authState == 'authenticated' && myControl.extensions.convertSessionToOrder.utilities.addressListOptions('bill') != false)	{
//					myControl.util.dump(" -> user is logged in AND has predefined billing address(es)");
					...
					txt += myControl.extensions.convertSessionToOrder.utilities.addressListOptions('bill'); // creates pre-defined address blocks.
...
//update form elements based on cart object.
				if(authState == 'authenticated' && myControl.extensions.convertSessionToOrder.utilities.addressListOptions('ship') != false)	{
...
//				myControl.util.dump('END myControl.extensions.convertSessionToOrder.panelContent.billAddress.');
				}, //billAddress
				
				
			shipAddress : function()	{
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.panelContent.shipAddress.  ');
				var txt = '';
				...
				if(myControl.sharedCheckoutUtilities.determineAuthentication() == 'authenticated' && myControl.extensions.convertSessionToOrder.utilities.addressListOptions('ship') != false)	{
...
// for existing customers/addresses, there is a default bill and a default ship address that could be different. So, the checkbox for bill to ship is NOT checked and the ship address panel is displayed.
					...
					txt += myControl.extensions.convertSessionToOrder.utilities.addressListOptions('ship'); // creates pre-defined address blocks.
					cssClass = 'displayNone'; 
					}
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.panelContent.shipMethods');
...
*/
				...
				if(foundMatchingShipMethodId == false)	{
					...
					myControl.extensions.convertSessionToOrder.calls.getCartContents.init('updateCheckoutOrderContents');
					myControl.model.dispatchThis('priorityDispatchQ');
					}
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.panelContent.cartContents');
...
//				myControl.util.dump('END myControl.extensions.convertSessionToOrder.panelContent.cartContents');
				}, //cartContents



			paymentOptions : function()	{
//				myControl.util.dump('myControl.extensions.convertSessionToOrder.panelContent.paymentOptions has been executed');
				...
				myControl.extensions.convertSessionToOrder.utilities.updatePayDetails(myControl.data.showCart.cart['chkout.payby']);
				}, //paymentOptions
...
*/
			orderNotes : function()	{
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.panelContent.orderNotes');
...
//				myControl.util.dump('END myControl.extensions.convertSessionToOrder.panelContent.orderNotes');
				} //orderNotes
...
//							myControl.util.dump(" -> no address selected. using default. ");
						...
						r += "' data-addressClass='"+TYPE+"' data-addressId='"+a['_id']+"' onClick='myControl.extensions.convertSessionToOrder.utilities.selectPredefinedAddress(this);' id='"+TYPE+"_address_"+a['_id']+"'>";
...
//state, zip and country may not be populated. check so 'undef' isn't written to screen.
					...
					r += "<address class='pointer' onClick='$(\"#"+TYPE+"AddressUL\").toggle(true); myControl.extensions.convertSessionToOrder.utilities.removeClassFromChildAddresses(\""+parentDivId+"\");'>Enter new address or edit selected address<\/address>";
...
_gaq.push(['_trackEvent','Checkout','App Event','Empty Cart Message Displayed']);
				...
				$('#zContent').empty().append("<p>It appears your cart is empty. If you think you are receiving this message in error, please contact the site administrator.<\/p>");					
				}, //cartIsEmptyWarnin
...
//if checkout succeded but payment failed (cash, cc fail, PO, etc) then this function gets executed.
			checkoutSuccessPaymentFailure : function(paycode,payby)	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.utilities.checkoutSuccessPaymentFailure');
...
// not used yet. can/will be used for cross selling
			getProdDataForCartItems : function()	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.utilities.getProdDataForCartItems');
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.utilities.handleCreateAccountCheckbox');
...
//				myControl.util.dump('END myControl.extensions.convertSessionToOrder.utilities.handleCreateAccountCheckbox');
...
//executed when a coupon is submitted. handles ajax call for coupon and also updates cart.
			handleCouponSubmit : function()	{
				...
				$('#addCouponBtn').attr('disabled','disabled').addClass('disabled loadingButtonBg');
				myControl.extensions.convertSessionToOrder.calls.addCouponToCart.init($('#couponCode').val(),'addCouponToCart'); 
				myControl.extensions.convertSessionToOrder.calls.getCartContents.init('updateCheckoutOrderContents');
				myControl.model.dispatchThis('priorityDispatchQ');
				}, //handleCouponSubmit
...
//panel is reloaded in case the submission of a gift card changes the payment options available.
			handleGiftcardSubmit : function()	{
				myControl.extensions.convertSessionToOrder.calls.addGiftcardToCart.init($('#giftcardCode').val(),'addGiftcardToCart'); 
				myControl.extensions.convertSessionToOrder.calls.getPaymentMethodsForCheckout.init();
				myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutPayOptions');
				myControl.extensions.convertSessionToOrder.calls.getCartContents.init('updateCheckoutOrderContents');
				myControl.model.dispatchThis('priorityDispatchQ');
...
//				var paymentID = $("[name='chkout.payby']:checked").val(), o = '';
				$('#chkoutPayOptionsFieldsetErrors').empty().toggle(false); //clear any existing errors from previously selected payment method.
				$('#chkout-payOptions li').removeClass('selected  ztab1');
				$('#payby_'+paymentID).addClass('selected  ztab1');
				$('#chkout-payOptions .paybySupplemental').addClass('displayNone'); //hide all other payment messages/fields.
	...
	//only add the 'subcontents' once. if it has already been added, just display it (otherwise, toggling between payments will duplicate all the contents)
				if($selectedPayment.length == 0)	{
					$selectedPayment = $("<ul />").attr("id","paybySupplemental_"+paymentID).addClass("paybySupplemental noPadOrMargin noListStyle");
...
//expiration is less of a concern
						case 'CREDIT':
							o = "<li><label for='payment-cc'>Credit Card #<\/label><input type='text' size='2' name='payment.cc' id='payment-cc' class='zform_textbox creditCard' value='";
							if(myControl.extensions.convertSessionToOrder.vars['payment.cc']){o += myControl.extensions.convertSessionToOrder.vars['payment.cc']}
							o += "' onKeyPress='return myControl.util.numbersOnly(event);' onChange='myControl.extensions.convertSessionToOrder.vars[\"payment.cc\"] = this.value' /><\/li>";
							o += "<li><label>Expiration<\/label><select name='payment.mm' id='payment-mm' class='creditCardMonthExp zform_select' onChange='myControl.extensions.convertSessionToOrder.vars[\"payment.mm\"] = this.value;'><option><\/option>";
							o += myControl.util.getCCExpMonths(myControl.data.showCart.cart['payment.mm']);
							o += "<\/select>";
							o += "<select name='payment.yy' id='payment-yy' class='zform_select creditCardYearExp' onChange='myControl.extensions.convertSessionToOrder.vars[\"payment.yy\"] = this.value;'><option value=''><\/option>"+myControl.util.getCCExpYears(myControl.data.showCart.cart['payment.yy'])+"<\/select><\/li>";
							o += "<li><label for='payment.cv'>CVV/CID<\/label><input type='text' size='4' name='payment.cv' id='payment-cv' class='zform_textbox creditCardCVV' onKeyPress='return myControl.util.numbersOnly(event);' value='";
							if(myControl.extensions.convertSessionToOrder.vars['payment.cv']){o += myControl.extensions.convertSessionToOrder.vars['payment.cv']}
							o += "' onChange='myControl.extensions.convertSessionToOrder.vars[\"payment.cv\"] = this.value' /><\/li>";
							break;
	
						case 'PO':
							o = "<li><label for='payment-po'>PO #<\/label><input type='text' size='2' name='payment.po' id='payment-po' class='zform_textbox purchaseOrder' onChange='myControl.calls.setSessionVars.init({\"payment.po\":this.value});' value='";
...
//the info below is added to the pdq but not immediately dispatched because it is low priority. this could be changed if needed.
								o += "<li><label for='"+safeid+"'>"+myControl.extensions.convertSessionToOrder.vars.echeck[key]+"<\/label><input type='text' size='2' name='"+key+"' id='"+safeid+"' class='zform_textbox echeck' onChange='myControl.calls.setSessionVars.init({\""+key+"\":this.value});' value='";
				...
				$selectedPayment.removeClass('displayNone'); //show focus payment message/field
	//			myControl.util.dump('END myControl.extensions.convertSessionToOrder.utilities.updatePayDetails. paymentID = '+paymentID);			
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.utilities.showServerErrors. there are '+L+' errors');
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.utilities.handlePanel.');
...
//output which is automatically added to each panel. Used for title and error handling.
				var o = "<legend id='"+panel+"Legend' class='zbox_head'>"+myControl.extensions.convertSessionToOrder.vars.legends[panel]+"<\/legend>";
...
//					myControl.util.dump(' -> panel does not exist');
					$('#zCheckoutFrm').append("<fieldset id='"+panel+"Fieldset' class='zbox zbox_body loadingBG "+cssClass+"'>"+o+"<\/fieldset>");
					}
				else{
//					myControl.util.dump(' -> panel exists');
					$('#'+panel+'Fieldset').empty().addClass('zbox zbox_body loadingBG '+cssClass).append(o);
					}
//				myControl.util.dump('END myControl.extensions.convertSessionToOrder.utilities.handlePanel.');				
				}, //handlePanel
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.utilities.');	
//				myControl.util.dump('value = '+shipID);	
				myControl.calls.setSessionVars.init({'ship.selected_id':shipID});
				myControl.extensions.convertSessionToOrder.calls.getCartContents.init('updateCheckoutOrderContents'); //update cart totals to reflect new ship method selected.
				myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutPayOptions');  //empty panel and set to loading
				myControl.extensions.convertSessionToOrder.calls.getPaymentMethodsForCheckout.init("updateCheckoutPayOptions"); //updates payment panel (pay methods may change based on shipping method)
...
//if the safeid isn't passed, we're likely in a select list or some other format where style changes are not needed.
				if(safeID)	{
					$("#chkout-shipMethods li").removeClass("selected ztab1");
					$('.shipcon_'+safeID).addClass('selected ztab1');
					}
...
//				myControl.util.dump("BEGIN myControl.extensions.convertSessionToOrder.utilities.selectPredefinedAddress");
				var $x = $(addressObject);
				...
				myControl.extensions.convertSessionToOrder.utilities.removeClassFromChildAddresses($x.parent().attr('id'));
				$x.addClass('selected  ztab1');
...
*/
				myControl.extensions.convertSessionToOrder.calls.saveCheckoutFields.init(); 

//for billing addresses, the payment panel must be updated.
				if(addressClass == 'bill')	{
					myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutPayOptions'); //empty panel. set to loading.
					myControl.extensions.convertSessionToOrder.calls.getPaymentMethodsForCheckout.init("updateCheckoutPayOptions");
					}
//for shipping addresses, the shipping methods panel needs updating. if predefined addresses exist, no 'ship to bill' checkbox appears.
					...
					myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutShipMethods'); //empty panel. set to loading.
					myControl.extensions.convertSessionToOrder.calls.getShippingRatesWithUpdate.init("updateCheckoutShipMethods"); //update shipping methods and shipping panel
					}
				else	{
					myControl.util.dump(" -> UNKNOWN class for address selection. should be bill or ship. is: "+addressClass);
					}

				myControl.extensions.convertSessionToOrder.calls.getCartContents.init('updateCheckoutOrderContents');  //updates cart object and reloads order contents panel.
				myControl.model.dispatchThis('priorityDispatchQ');
...
//executed when the 'bill to ship' checkbox is checked (either on or off)
			toggleShipAddressPanel : function()	{
				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.utilities.toggleShipAddressPanel');
//ship to billing
				if($('#chkout-bill_to_ship_cb').is(':checked')) {
					...
					myControl.extensions.convertSessionToOrder.calls.saveCheckoutFields.init(); //update session. all fields are updated because shipping address fields were populated.
					myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutShipMethods');  //empties panel. sets to loading.
					myControl.extensions.convertSessionToOrder.calls.getShippingRatesWithUpdate.init("updateCheckoutShipMethods"); //update shipping methods and shipping panel
					myControl.extensions.convertSessionToOrder.calls.getCartContents.init('updateCheckoutOrderContents');  //updates cart object and reloads order contents panel.
					}
//do not ship to billing
					...
					myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutShipAddress');  //empties panel. sets to loading.
					$('#chkoutShipAddressFieldset').toggle(true);  //make sure panel is visible.
					myControl.extensions.convertSessionToOrder.panelContent.shipAddress(); //populate panel.
...
//				myControl.util.dump("BEGIN myControl.extensions.convertSessionToOrder.utilities.addressFieldUpdated");
				if(myControl.extensions.convertSessionToOrder.utilities.taxShouldGetRecalculated())	{
					myControl.extensions.convertSessionToOrder.calls.getCartContents.init('updateCheckoutOrderContents');
					}
				}, //addressFieldUpdated
...
//this function checks to see if they're populated and, if so, returns true.
			taxShouldGetRecalculated : function()	{
				myControl.util.dump("BEGIN myControl.extensions.convertSessionToOrder.utilities.taxShouldGetRecalculated");
				var r = true;
				var errors = 0;
...
_gaq.push(['_trackEvent','Checkout','App Event','Gathered enough input to display tax']);
				...
				myControl.util.dump("END myControl.extensions.convertSessionToOrder.utilities.taxShouldGetRecalculated");
				return r;
...
//the saveCheckoutFields above this if statement needs to be put in the q prior to the getPaymentMethods below.
					if(billToShip) {
						myControl.extensions.convertSessionToOrder.utilities.handlePanel('chkoutPayOptions'); //empties panel. sets to loading.
						myControl.extensions.convertSessionToOrder.calls.getPaymentMethodsForCheckout.init("updateCheckoutPayOptions");
						}

					myControl.extensions.convertSessionToOrder.calls.getShippingRatesWithUpdate.init("updateCheckoutShipMethods");
					}
				}, //recalculateShipMethods


//will remove the selected and ztab1 classes from all address elements within the passed parent div id.
			removeClassFromChildAddresses : function(parentDivId)	{
				$('#'+parentDivId+' address').each(function() {
					$(this).removeClass('selected  ztab1');
					});				
...
//				myControl.util.dump("BEGIN myControl.extensions.convertSessionToOrder.renderFormats.countriesAsOptions");
				var r = '';
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.renderFormats.secureLink');
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.renderFormats.orderStatusLink');
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.formats.shipMethodsAsOptions');
				var o = '';
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.formats.shipMethodsAsRadioButtons');
				var o = '';
				var shipName;
				var L = data.value.length;
...
//myControl.util.dump(' -> id = '+id+' and ship.selected_id = '+myControl.data.showCart.cart['ship.selected_id']);
					...
					if(isSelectedMethod)
						o+= ' ztab1 selected ';
					o += "shipcon_"+safeid; //hhhmmm... seems to cause issues sometimes. add it last so ztab1 and selected always get added.
					o += "'><input type='radio' name='ship.selected_id' id='ship-selected_id_"+safeid+"' value='"+id+"' onClick='myControl.extensions.convertSessionToOrder.utilities.updateShipMethod(this.value,\""+safeid+"\"); myControl.model.dispatchThis(\"priorityDispatchQ\"); '";
					if(isSelectedMethod)
						o += " checked='checked' "
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.renderFormats.payOptionsAsRadioButtons');
//				myControl.util.dump(data);
				var L = data.value.length;
				var o = "";
				var id = '';
...
//paypalec is not supported at this time.						
						...
						o += "<li class='paycon paycon_"+id+"' id='payby_"+id+"'><input type='radio' name='chkout.payby' id='chkout-payby_"+id+"' value='"+id+"' onClick='myControl.extensions.convertSessionToOrder.utilities.updatePayDetails(this.value); myControl.calls.setSessionVars.init({\"chkout.payby\":this.value}); myControl.model.dispatchThis(\"priorityDispatchQ\"); $(\"#chkoutPayOptionsFieldsetErrors\").addClass(\"displayNone\");' ";
						if(isSelectedMethod)
							o += " checked='checked' "
						o += "/><label for='chkout-payby_"+id+"'>"+data.value[i].pretty+"<\/label><\/li>";
						}
					}
...
//				myControl.util.dump('BEGIN myControl.extensions.convertSessionToOrder.renderFormats.accessoryUpsell');
//				myControl.util.dump(data);
